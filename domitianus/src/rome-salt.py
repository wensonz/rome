...
HTTP_TIMEOUT = 30
API_HOSTNAME = "api.rome.cluster.sina.com.cn"
API_PORT = "8080"
API_HOSTADDR = socket.gethostbyname(API_HOSTNAME)
...
def request(url, data=None, post_json=False, accept_json=False, out=None): #{{{2
    ...
    global API_HOSTADDR = req.get_host()
...
def syn_sls(generate_result, node_name): # {{{2
    """
    ...
    rsync_path = "%s::salt/%s/*" % (API_HOSTADDR, node_name)
...
def main(): # {{{2
    ...
    if sync:
        syn_sls(generate("http://%s:%s/generate" % (API_HOSTNAME, API_PORT), get_node_name()))
